...
public class SimpleDataEncountered extends AbstractModelConstraint {

	public SimpleDataEncountered() {
		super();
	}
	...
	public IStatus validate(IValidationContext ctx) {
		if (ctx.getTarget() instanceof ESettingData) {
			...
			if (ptr instanceof EAppTypeContainer) {
				if (settingData.getText() != null
						&& settingData.getText().trim().length() > 0) {
			...
			com.nokia.tools.vct.confml.model.confml2.ESettingData settingData = (com.nokia.tools.vct.confml.model.confml2.ESettingData) ctx
					.getTarget();
			StringBuilder tmp = new StringBuilder();
			EAppSettingType ptr = ConfMLUtils.findType(settingData, tmp);
			String typePath = tmp.toString();
