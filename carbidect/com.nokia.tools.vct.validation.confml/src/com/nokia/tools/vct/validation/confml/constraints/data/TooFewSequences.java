...
import com.nokia.tools.vct.confml.model.confml1.impl.EFeatureDataImpl;
import com.nokia.tools.vct.validation.confml.constraints.ConfMLUtils;

public class TooFewSequences extends AbstractModelConstraint {

	public TooFewSequences() {
		super();
	}
	...
	public IStatus validate(IValidationContext ctx) {
		...
		if (ctx.getTarget() instanceof ESettingData) {
			...
			if (ptr instanceof EAppSequenceType) {
				...
				Integer minOccurs = appSeqType.getMinOccurs();
				if (minOccurs != null)
				{
					EFeatureDataImpl featureData = (EFeatureDataImpl)settingData.eContainer();
					if (featureData != null && featureData.getSettings() != null)
					{
						int numberOfSequences = featureData.getSettings().size(); 
						if (numberOfSequences<minOccurs.intValue())	
							return ctx.createFailureStatus(typePath, minOccurs);
					}
				}
			}
		}
		else if (ctx.getTarget() instanceof com.nokia.tools.vct.confml.model.confml2.ESettingData)
		{
			...
			if (ptr instanceof EAppSequenceType) {
				...
				if (minOccurs != null)
				{
					com.nokia.tools.vct.confml.model.confml2.impl.EFeatureDataImpl featureData = (com.nokia.tools.vct.confml.model.confml2.impl.EFeatureDataImpl)settingData.eContainer();
					if (featureData != null && featureData.getSettings() != null)
					{
						int numberOfSequences = featureData.getSettings().size(); 
						if (numberOfSequences<minOccurs.intValue())	
