...
public final class ModelSearchParticipantArea extends
	...
	public List<AdapterFactory> getMetaElementComposeableAdapterFactories() {
		...
		if (EIBYModelPackage.eNS_URI.equals(nsURI)) {
			composeableAdapterFactories
					.add(new EIBYModelItemProviderAdapterFactory());
		}
	...
	protected Collection<EPackage> getTargetModelPackages() {
		...
		if (EIBYModelPackage.eNS_URI.equals(nsURI)) {
			return Arrays.asList(new EPackage[] { EIBYModelPackage.eINSTANCE });
		}

		return Arrays.asList(new EPackage[] {

		EIBYModelPackage.eINSTANCE
	...
	protected Image getTargetModelElementImage(Object object) {
		try {
			if (object instanceof ENamedElement) {
				...
				URL url = FileLocator
						.find(
								com.nokia.tools.vct.ibyml.ibymodel.provider.IBYMLModelEditPlugin
			...
			Activator
					...
					.log(
							new Status(
									Status.ERROR,
									Activator.PLUGIN_ID,
									"Error while attempmting to retrieve image from edit" + com.nokia.tools.vct.ibyml.ibymodel.provider.IBYMLModelEditPlugin.getPlugin().getBundle() + " bundle")); //$NON-NLS-1$  //$NON-NLS-2$
		}
		return null;
	}
