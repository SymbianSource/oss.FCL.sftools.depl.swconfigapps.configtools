...
import com.nokia.tools.s60ct.confml.editor.converter.VEModel2ConfML1PMConverter;
...
public class VEModel2Confml1PMConverterTest extends TestCase{

	private VEModel2ConfML1PMConverter converter;
	private ConfmlFactory factory;

	@Override
	protected void setUp() throws Exception {
		converter = new VEModel2ConfML1PMConverter();
	...
	public void testEFeatureWithSequenceSetting() throws Exception {
		...
		EConfMLDocument document = (EConfMLDocument) resource.getContents()
				.get(0);
		EConfigurationElement configuration = document.getConfiguration();
		EList<EDataElement> data = configuration.getData();
		assertFalse("There must be data element in the configuration",
				data.isEmpty());
		...
		assertEquals(2, fData.getSettings().size());
//		resource.save(Collections.EMPTY_MAP);
	}

	private Resource createResource(ResourceSet rs) {
		rs.getResourceFactoryRegistry().getExtensionToFactoryMap().put(
				"confml", new EConfML1ResourceFactoryImpl());
		URI uri = URI.createFileURI(new File("sequencetest1.confml")
				.getAbsolutePath());
		Resource resource;
		try {
