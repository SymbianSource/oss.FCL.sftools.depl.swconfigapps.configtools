...
import com.nokia.tools.variant.validation.core.MarkerCategory;
import com.nokia.tools.variant.validation.core.MessageLocation;
import com.nokia.tools.variant.validation.core.ValidationContext;
import com.nokia.tools.variant.validation.core.ValidationMessage;
	...
	protected ValidationResult updateProjectMetadata(ValidationContext context,
		...
		try {
			...
			try {
				...
				ValidationMessage message = new ValidationMessage(4, metaLoc,
						ConfmlMarkerUtils.CONFML_PROBLEM_MARKER,
						MarkerCategory.LAYER);
				result.addMessage(message);
				return result;
...
//				result.addMessage(message);
//			}
			if (!isValidLayerName(metadata.getName())) {
				// WARNING - continue
				ValidationMessage message = new ValidationMessage(6, metaLoc,
						ConfmlMarkerUtils.CONFML_PROBLEM_MARKER,
						MarkerCategory.LAYER);
				result.addMessage(message);
			}
