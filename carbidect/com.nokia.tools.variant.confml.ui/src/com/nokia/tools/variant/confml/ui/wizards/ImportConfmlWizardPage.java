...
public class ImportConfmlWizardPage extends WizardPage {
	...
	final class ImportOptions extends Observable implements IDialogFieldListener {
		protected final SelectionButtonDialogField fixXIncludeCheck;
		protected final SelectionButtonDialogField prettyPrintCheck;
		protected final SelectionButtonDialogField makeLinks;

		public ImportOptions() {
			fixXIncludeCheck = new SelectionButtonDialogField(SWT.CHECK);
			fixXIncludeCheck.setDialogFieldListener(this);
			fixXIncludeCheck.setLabelText("Fix XInclude directives (namespace and href)");
			fixXIncludeCheck.setSelection(true);
			prettyPrintCheck = new SelectionButtonDialogField(SWT.CHECK);
			prettyPrintCheck.setDialogFieldListener(this);
			prettyPrintCheck.setLabelText("Format XML (pretty print + UTF8)");
			prettyPrintCheck.setSelection(true);
...
//			makeLinks.setToolTipText("Create links instead of copying files");
		...
		public void dialogFieldChanged(DialogField field) {
			if(field==makeLinks && makeLinks.isSelected()) {
				fixXIncludeCheck.setSelection(false);
				prettyPrintCheck.setSelection(false);
				fixXIncludeCheck.setEnabled(false);
				prettyPrintCheck.setEnabled(false);
			}
			if(field==makeLinks && !makeLinks.isSelected()) {
				fixXIncludeCheck.setEnabled(true);
				prettyPrintCheck.setEnabled(true);
			}
			fireEvent();
		}
		...
		public Control createControl(Composite composite) {
			...
			fixXIncludeCheck.doFillIntoGrid(group, numColumns);
			prettyPrintCheck.doFillIntoGrid(group, numColumns);
			makeLinks.doFillIntoGrid(group, numColumns);

			LayoutUtil.setHorizontalGrabbing(fixXIncludeCheck
					.getSelectionButton(null));
			LayoutUtil
			.setHorizontalGrabbing(prettyPrintCheck.getSelectionButton(null));
	...
	public ImportConfmlWizardPage() {
		super("ImportConfMLPageOne");
		setTitle("ConfML Import Wizard"); // NON-NLS-1
		setDescription("Import a file or set of ConfMLs from the local file system into the workspace"); // NON
																											// -
																											// NLS
																											// -
																											// 1
	...
	public boolean isFixXInclude() {
		return importOptions.fixXIncludeCheck.isSelected();
	}
	public boolean isPrettyPrint() {
		return importOptions.prettyPrintCheck.isSelected();
	}
