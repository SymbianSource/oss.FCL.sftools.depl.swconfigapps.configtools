...
package com.nokia.tools.vct.internal.common.ui.navigator;
...
public class NavigatorButton extends Item {
	...
	public NavigatorButton(NavigatorComposite parent) {
		...
		fontColorBold = colorRegistry
				.get("com.nokia.tools.vct.common.ui.navigator.FontColorBold");
		// borderHighlightColor = colorRegistry
		// .get("com.nokia.tools.vct.common.ui.navigator.BorderHighlightColor");
		borderShadowColor = colorRegistry
				.get("com.nokia.tools.vct.common.ui.navigator.BorderShadowColor");
		labelFocusColor = colorRegistry
				.get("com.nokia.tools.vct.common.ui.navigator.LabelFocusColor");
		...
		this.buttonDescriptionText = new MultiLineText(parent,
				SWT.NO_BACKGROUND);
		...
		this.buttonNameText.setForeground(fontColorBold);
	...
	private void layout() {
		buttonDescriptionText.setBounds(new Rectangle(x + MAX_LABEL_WIDTH + 5,
				y + 5, width - MAX_LABEL_WIDTH - 10, height - 10));
		;

		Rectangle bounds = new Rectangle(x + 5, y + 5, MAX_LABEL_WIDTH - 10,
				height - 10);
		Point nameSize = buttonNameText.computeSize(MAX_LABEL_WIDTH - 10,
				SWT.DEFAULT);
		...
		bounds.x = x + 5;
		bounds.y = y + (height - nameSize.y - iconBounds.height) / 2
				+ iconBounds.height;
	...
	public Point computeSize(int wHint, int hHint) {
		...
		if (expanded && (wHint == SWT.DEFAULT || wHint > MAX_LABEL_WIDTH)) {
			...
			Point descriptionSize = buttonDescriptionText.computeSize(
					wHint == SWT.DEFAULT ? SWT.DEFAULT : wHint
							- MAX_LABEL_WIDTH - 10, hHint);
	...
	private Point computeLeftAreaSize(int wHint, int hHint) {
		...
		Point nameSize = buttonNameText
				.computeSize(MAX_LABEL_WIDTH - 10, hHint);
	...
	public void paintControl(PaintEvent e) {
		...
		if (icon != null) {
			...
			gc.drawImage(icon, x + (MAX_LABEL_WIDTH - bounds.width) / 2, y
					+ (height - bounds.height - nameBounds.height - 5) / 2);
		}
		...
		gc.setForeground(borderShadowColor);
	...
	public void setSelected(boolean selected) {
		...
		Color background = selected ? labelFocusColor : navigator
				.getBackground();
