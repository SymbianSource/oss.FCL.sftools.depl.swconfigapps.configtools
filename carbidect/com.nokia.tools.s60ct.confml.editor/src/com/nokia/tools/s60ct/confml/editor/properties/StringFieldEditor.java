...
public class StringFieldEditor extends LabelFieldEditor {
	...
	public void fillIntoGrid(Composite parent, int columns, int rows, FormToolkit toolkit) {
		super.fillIntoGrid(parent, 1, toolkit);
		...
		if ((style & SWT.MULTI) != 0) {
			gridData.minimumHeight = pixelConverter.convertHeightInCharsToPixels(rows);
			gridData.heightHint = pixelConverter.convertHeightInCharsToPixels(rows);
			//gridData.widthHint = 100;
