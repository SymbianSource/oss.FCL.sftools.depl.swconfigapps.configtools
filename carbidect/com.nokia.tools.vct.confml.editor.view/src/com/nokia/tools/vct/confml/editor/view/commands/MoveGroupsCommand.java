...
public class MoveGroupsCommand extends Command {
	...
	private List<EGroup> objectsToMove;
	...
	public MoveGroupsCommand(ViewsEditor editor, EGroupContainer newParent, int index,  List<EGroup> objs) {
			...
			domain.getCommandStack().execute(new RecordingCommand(domain) {
				...
				protected void doExecute() {
					...
					for (int i = 0; i < objectsToMove.size(); i++) {
						EGroup group = objectsToMove.get(i);
						EGroupContainer container = (EGroupContainer)group.eContainer();
						parentsOfObjectsToRemove.add(container);
						int indexOfChildInParent = container.getGroupsAndViews().indexOf(group);
	...
	public void undo() {
		...
		try {
			...
			for (int i = objectsToMove.size() - 1; i >= 0; i--) {
				...
				EGroup group = objectsToMove.get(i);
				parent.getGroupsAndViews().add(index, group);
			}
				}});
			domain.getCommandStack().flush();
