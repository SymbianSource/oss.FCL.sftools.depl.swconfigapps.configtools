...
primaryExpr: 
  settingValueRef
  | '(' orExpr ')'
  | l = Literal {String literal = $l.getText().substring(1, $l.getText().length() - 1) ; $l.setText(literal);}
  | IntegerNumber 
  | RealNumber 
  | ScientificNumber
;
...
subSettingRef:
  r1 = Ref '/' rx = Ref '[' n = IntegerNumber ']' '/' rs = Ref
...
ScientificNumber : 
  ( PLUS | MINUS ) IntegerNumber  Exponent
  ;
