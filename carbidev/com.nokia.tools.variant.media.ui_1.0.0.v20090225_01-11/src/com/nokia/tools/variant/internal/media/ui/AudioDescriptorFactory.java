...
import com.nokia.tools.variant.media.ui.preview.AudioDescriptor;
...
public class AudioDescriptorFactory implements IPreviewDescriptorFactory{

	public IPreviewDescriptor createPreviewDescriptor(IStorage source, IContentDescription description) {
		try {
			return new AudioDescriptor(description.getContentType(), source);
		} catch (Exception ex) {
			MediaUIPlugin.logError(
					"Failed to construct audio descriptor", ex);
