...
package com.nokia.tools.variant.resourcelibrary.handlers;
		...
		if (IS_ROOT.equals(property)) {
			...
			if (receiver instanceof ResourceStorage){
				...
				if (readOnly) {
					...
					FileSystemElement fse = rs.getFileSystemElement();
			...
			if (receiver instanceof ResourceStorage){
				ResourceStorage rs = (ResourceStorage) receiver;
				return rs.getFileSystemElement() instanceof Directory;
			}
		}
