...
import com.nokia.tools.variant.views.errorview.ErrorsViewPage;
 ...
 * provider and input. At the end you have to call populateTableViewer. Simple
 * example, you can see in {@link ErrorsViewPage}.
 * 
 * @see Column
 */
	...
	public void addColumn(Column column) {
		...
		genericColumn.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				ViewerFilter[] filters = getTv().getFilters();
				if (filters.length > 0) {
					for (ViewerFilter viewerFilter : filters) {
						if (viewerFilter instanceof ComboViewerFilter) {
							ComboViewerFilter cvf =  (ComboViewerFilter) viewerFilter;
							cvf.setCounter(0);
						}
					}
				}
				getTv().setSorter(new Sorter(currentColumn, dir));
			}
